...
const { pending, data, error } = $api.lazyFetchForm(`/trains/${route.params.id}`, form.fields, (newFields) => {
...
const submit = async (data) => {
    try {
        ...
        const res = await $api.put(`/trains/${route.params.id}`, dataToSend)
...
<template>
    <PageWrapper :pending="pending" :error="error" errorText="Unable to load train details.">
        ...
        <div class="max-w-screen-md mx-auto">
            <AlertError v-if="state.error" text="Unable to update the train." />
